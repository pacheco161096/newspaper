...
PostgreSQL, CMS nativo, descubrimiento, clasificación y resolución de eventos ya están en producción. Falta la redacción con IA (prompt por aprobar) y Zernio.

## Descubrimiento por cron-job.org

`POST /api/cron/discover` recibe `Authorization: Bearer <CRON_SECRET>`. Sin parámetro procesa las fuentes habilitadas; con `?source=noticias_pv`, `?source=tribuna_bahia`, `?source=el_universal` o `?source=record` ejecuta únicamente una.
...
Cada ejecución crea un registro en `ingestion_runs`. Los documentos se guardan por huella única en `source_documents`; repetir el cron no duplica el contenido. Noticias PV usa WordPress REST. El Universal usa el sitemap de Google News, pasa por un prefiltro temático y se limita a diez documentos **nuevos** por día. Récord usa `sitemap-news-latest.xml`, hidrata el HTML público de cada nota (tope 5 nuevas/día, solo deporte) y no consulta `/api`. Tribuna está configurada con su `news-sitemap.xml` pero **pausada** (HTTP 403 desde Vercel).

La siguiente etapa es el prompt editorial. Antes de la redacción con IA se debe aprobar el prompt y su salida JSON; ningún prompt editorial está definido todavía.

## CMS nativo

El panel vive en `/admin` dentro de Next.js. Permite crear, editar, previsualizar, publicar y despublicar noticias. Incluye campos editoriales, imagen, SEO, texto para Facebook y trazabilidad de la fuente. El autor público no es editable: siempre es **Hola Vallarta**. Cada cambio importante guarda una revisión auditable en `cms.article_revisions`.
